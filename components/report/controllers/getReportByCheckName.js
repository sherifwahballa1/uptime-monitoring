...
ReportsByCheckName = catchAsync(async (req, res, next) => {
    ...
    const check = await Check.findOne({ name: checkName, userId: mongoose.Types.ObjectId(req.userData._id) });

    if (!check) return next(createError(404, "Check not found"));

    const reports = await Report.find({ checkId: mongoose.Types.ObjectId(check.checkId) });
