...
editCheck = catchAsync(async (req, res, next) => {
  ...
  if (value && Object.keys(value).length === 0)
    return next(createError(400, "There's no exists something to update"));
  ...
  if (value.name) {
    ...
    if (checks.length > 0)
      return res.status(429).json({ message: "check name used before" });
  }

  const updatedCheck = await Check.findByIdAndUpdate(
    { _id: checkID, userId: req.userData._id },
    value,
    {
      new: true,
    }
