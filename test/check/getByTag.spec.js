...
describe("GET ALL CHECK DATA GET api/check/tag/${tagName}", () => {
  let name;
  let userData;
  let tempToken;
  let otp;
  ...
  describe("GET All Checks By TagName", () => {
    describe("get all checks by tag", () => {
      describe("Without credentials", () => {
        it("try to create without headers (Authorization)", () => {
          return request
            .get(`/api/check/tag/tagNames`)
            .send()
            .then((response) => {
      ...
      describe("Checks by tagName", () => {
        it("invalid query params", () => {
          return request
            .get(`/api/check/tag/x?limitNo=-20`)
            .set({ authorization: token })
            .send()
            .then((response) => {
              expect(response.status).to.equal(400);
