...
describe("Delete ALL CHECK DATA BY tagName DELETE api/check/tag/${tagName}", () => {
  let name;
  let userData;
  let tempToken;
  let otp;
  ...
  describe("Delete All Checks By TagName", () => {
    describe("remove all checks by tag", () => {
      describe("Without credentials", () => {
        it("try to create without headers (Authorization)", () => {
          return request
            .delete(`/api/check/tag/tagNames`)
            .send()
            .then((response) => {
      ...
      describe("Checks by tagName", () => {
        it("tagName not exists query params", () => {
          return request
            .delete(`/api/check/tag/VBV`)
            .set({ authorization: token })
            .send()
            .then((response) => {
              ...
              expect(response.body)
                .to.have.property("message")
                .to.equal("no checks founded");
            });
        });
        it("Remove Checks", () => {
          return request
            .delete(`/api/check/tag/x`)
            .set({ authorization: token })
            .send()
            .then((response) => {
              expect(response.status).to.equal(200);
