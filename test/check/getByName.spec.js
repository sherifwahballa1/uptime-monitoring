...
describe("GET CHECK DATA by name get api/check/name/${checkName}", () => {
  let name;
  let userData;
  let tempToken;
  let otp;
  ...
  describe("GET the check by name", () => {
    describe("get the check", () => {
      describe("Without credentials", () => {
        it("try to create without headers (Authorization)", () => {
          return request
            .get(`/api/check/name/aaaa`)
            .send()
            .then((response) => {
      ...
      describe("with invalid check name", () => {
        it("invalid check name", () => {
          return request
            .get(`/api/check/name/$%%%%`)
            .set({ authorization: token })
            .send()
        ...
        describe("with valid check name", () => {
          it("Check info", () => {
            return request
              .get(`/api/check/name/${checkData.name}`)
              .set({ authorization: token })
              .send()
              .then((response) => {
                expect(response.status).to.equal(200);
